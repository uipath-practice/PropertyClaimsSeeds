...
- **Write it to `method/template-sdd-case.md`, and that template exists because no shipped one does both jobs.** Its four `## Section N:` headings are what the case build actually reads; the handoff header above them — the marker, the status, the open review items — is what makes the design reviewable and what the gate checks. The `uipath-maestro-case` template has the first half, `uipath-planner`'s has the second, and **a design in the wrong shape is never rejected — it is built thinly and validates clean**: blank task inputs, exit rules that cannot fire, an SLA silently dropped.
